...
const Cart = () => {
    ...
    return (
        <div className="bg-half-transparent w-full fixed nav-item top-0 right-0 shadow-lg ">
            <div className="float-right h-screen  duration-1000 ease-in-out dark:text-gray-200 transition-all dark:bg-[#484B52] bg-white md:w-400 p-8">
                ...
                {cartData?.map((item, index) => (
                    <div key={index}>
                        <div>
                            <div className="flex items-center   leading-8 gap-3 sm:gap-5 border-b-1 border-color dark:border-gray-600 sm:p-4">
                                <img className="rounded-lg sm:h-80 h-[50px] w-[50px] sm:w-24" src={item.image} alt="" />
                                <div>
                                    <p className="font-semibold ">{item.name}</p>
                                    <p className="text-gray-600 dark:text-gray-400 text-sm font-semibold">{item.category}</p>
                                    <div className="flex gap-4 sm:mt-2 items-center">
